...
// Parse Google Sheets rows — handles trailing spaces in header names
export function parseSheetRows<T>(rows: string[][]): T[] {
  if (!rows || rows.length < 2) return [];
  const headers = rows[0].map(h => s(h)); // trim handles trailing space e.g. "Buyer "
  return rows.slice(1)
    ...
    .map(row => {
      ...
      headers.forEach((key, i) => {
        ...
        else if (!isNaN(Number(str)) && str !== "") obj[key] = Number(str);
        else obj[key] = str;
      });
      return obj as T;
    });
...
// Convert decimal DHU (0.036) to percent (3.6) — Finishing sheet stores as decimal
...
export function computeDashboard(
  ...
  const defectCatMap = new Map(
    defectMaster.map(d => [s(d.Defect_Name).toLowerCase(), s(d.Category).trim()])
  );
  ...
  // Defect lookup: HQ_Row_ID → defects[]
  ...
  for (const d of todayDefects) {
    ...
    const cat = s(d.Defect_Category) || defectCatMap.get(name.toLowerCase()) || "Other";
    const cur = defTotals.get(name) ?? {qty:0, cat};
  ...
  const hourlyTrend: HourlyTrend[] = Array.from(hrAcc.entries())
    ...
    .map(([hour,a]) => ({
      ...
      dhu: a.qc>0 ? +((a.def/a.qc)*100).toFixed(2) : 0,
      balance:a.bal,
    }));

  // ── Finishing — convert decimal DHU to percent ─────────────
  const finishingProcessed: FinishingRow[] = finishingRaw.map(r => ({
    ...r,
    DHU_Pct: toPercent(r.DHU_Pct),
    Target_DHU_Pct: toPercent(r.Target_DHU_Pct),
    Vs_Target_Pct: toPercent(r.Vs_Target_Pct),
  }));
  ...
  const remarks = today
    ...
    .map(r => ({
      line: s(r.Line_No), hour: s(r.Hour_Slot),
      qi: s(r.User_Name), text: s(r.Remarks), date: s(r.Date)
    }));
  ...
  return {
    kpis, lineDHU, controllerPerf, floorICPerf, defectBreakdown,
    hourlyTrend, hourlyRaw:today, finishingRaw:finishingProcessed,
    aoStyles, lineMapping, remarks
  };
}
