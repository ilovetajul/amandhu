...
export function fmtPct(v: number, d = 2) { return `${v.toFixed(d)}%`; }
export function fmtNum(v: number) { return new Intl.NumberFormat("en-US").format(v); }
...
export function statusGlow(s: "critical" | "warning" | "good") {
  return { critical: "#ef4444", warning: "#f59e0b", good: "#22c55e" }[s];
}
