...
export default function FinishingAnalysis({ data, lineMapping }: { data: FinishingRow[]; lineMapping: LineMapping[] }) {
  ...
  const lmByLine = useMemo(() => new Map(lineMapping.map(l => [String(l.Line_No), l])), [lineMapping]);
  const units = useMemo(() => [...new Set(data.map(r => String(r.Unit_No ?? "")).filter(Boolean))].sort(), [data]);

  const enriched = useMemo(() => data.map(r => {
    // DHU% already converted to percent in parser
    const dhu = Number(r.DHU_Pct ?? 0);
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-[--color-border]/50">
              {["Unit","Line","AO / Style","Buyer · Color","Inspected","Defects","DHU%","Target","Status","LQ","Controller","Floor IC","Remarks"].map(h => (
