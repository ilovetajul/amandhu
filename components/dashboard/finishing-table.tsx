...
export default function FinishingTable({ data }: { data: FinishingRow[] }) {
  ...
  const enriched = useMemo(() => data.map(r => {
    const dhu = r["DHU%"] != null
      ? Number(r["DHU%"]) * (Number(r["DHU%"]) <= 1 ? 100 : 1)
      : Number(r.Inspected_Qty) > 0
        ? (Number(r.Defectives_gmts) / Number(r.Inspected_Qty)) * 100
        : 0;
    const target = r["Target_DHU%"] != null
      ? Number(r["Target_DHU%"]) * (Number(r["Target_DHU%"]) <= 1 ? 100 : 1)
      : 3.0;
  ...
  const columns = useMemo<ColumnDef<typeof filtered[0]>[]>(() => [
    { accessorKey:"Date", header:"Date",
      cell:({getValue})=><span className="text-xs font-mono text-[--color-muted]">{String(getValue()??"-")}</span> },
    { accessorKey:"Line_No", header:"Line",
      cell:({getValue})=><span className="font-bold text-cyan-400 font-mono">{String(getValue()??"-")}</span> },
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 border-b border-[--color-border]">
        <div>
          ...
          <p className="text-xs text-[--color-muted] mt-0.5">
            ...
            <span className="text-red-400 font-bold">{data.length-passCount} Fail</span>
            {" · "}{data.length} total
          </p>
        </div>
